...
function App() {
  ...
  const [selectedJokeIndex, setSelectedJokeIndex] = useState(0) // state pt index
  const [easyLevels, setEasyLevels] = useState(EASY_JOKES);

  return (
    <>
      <div className="container-btn">
          <button key={index} clasName="easy" handleClick={() => setSelectedJokeIndex(index)}>Not so good jokes</button>
